...
public class WorkshopController : Controller<MainMenu, WorkshopModel, WorkshopView>
{
    ...
    {
        model.level = town.workshopLevel;
    ...
    {
        if (model.cost <= playerData.gold && model.level < TownData.MAX_WORKSHOP_LEVEL)
        {
            return true;
        }
        else
    ...
    public void ApplyUpgrade()
    {
        ...
        model.level++;
        town.workshopLevel = model.level;
        model.cost += 100;
    ...
    public void UpdateFacilitySprite(int upgradeLevel)
    {
        ...
        Sprite sprite = Resources.Load<Sprite>("Sprites/Town/workshop_" + upgradeLevel);
