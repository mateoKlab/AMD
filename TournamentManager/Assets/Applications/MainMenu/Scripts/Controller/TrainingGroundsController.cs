...
public class TrainingGroundsController : Controller<MainMenu, TrainingGroundsModel, TrainingGroundsView>
{
    ...
    {
        model.level = town.trainingGroundsLevel;
        model.cost = model.level * 200;
        model.cooldown = 3f;
    ...
    {
        if (model.cost <= playerData.gold && model.level < TownData.MAX_TRAINING_GROUNDS_LEVEL)
        {
            return true;
        }
        else
    ...
    public void ApplyUpgrade()
    {
        ...
        model.level++;
        town.trainingGroundsLevel = model.level;
        model.cost += 100;
    ...
    public void UpdateFacilitySprite(int upgradeLevel)
    {
        ...
        Sprite sprite = Resources.Load<Sprite>("Sprites/Town/training_grounds_" + upgradeLevel);
