...
//		levelDic.Add (10, ld10);
		...
		TextAsset attachmentCsv = Resources.Load("Data/XpData/WarriorXP") as TextAsset;

		string[] rows = attachmentCsv.text.Split(new char[] {'\n'});	
		for(int ctr = 1; ctr < rows.Length-1; ctr++) {
			string[] values = rows[ctr].Split(new char[] {','});	// Parse line per comma
			LevelData ld = new LevelData();
			ld.requiredXP = int.Parse(values[1].ToString());
			ld.atkGrowth = float.Parse(values[2].ToString());
			ld.hpGrowth = float.Parse(values[3].ToString());
			ld.defGrowth = float.Parse(values[4].ToString());
			levelDic.Add(ctr, ld);
		}
